...
#define _assert(test) do { if (!(test)) { isFailed++; asprintf(&errorMessage, "%d. %s() "RED"FAILED" RESET ". In File: %s, Line: %d\n", tests_run + 1, __func__, __FILE__, __LINE__); tests_failed++; break; } PASS(); test_passed++; } while(0); tests_run++;
